...
class LibraryCLI:
    def __init__(self, service: LibraryService):
        self._io = LibraryIO(service)

    def run(self):
        while True:
            self._io._print_menu()
            choice = input("\nВаш вибір: ").strip()
            ...
            match choice:
                case "1":
                    self._io._add_book()
                case "2":
                    self._io._delete_book()
                case "3":
                    self._io._find_by_title()
                case "4":
                    self._io._find_by_author()
                case "5":
                    self._io._show_all_books()
                case "6":
                    self._io._mark_as_read()
                case "7":
                    self._io._show_statistics()
                case "0":
                    print("До побачення!")
                    break
                case _:
